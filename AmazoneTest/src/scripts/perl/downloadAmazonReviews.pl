...
# usage: ./downloadAmazonReviews.pl <list of IDs of amazon products>
# example: ./downloadAmazonReviews.pl B0040JHVC2 B004CG4CN4
# output: a directory ./amazonreviews/<ID> is created for each product ID; HTML files containing reviews are downloaded and saved in each directory.
...
while($id  = shift) {
    ...
    my $dir = "amazonreviews/$id";
    mkdir $dir;

    my $urlPart1 = "http://www.amazon.com/product-reviews/";
	...
	if($response->is_success) {
		...
		close(CONTENTFILE);
		print "ok\t$id\t$page\t$lastPage\n";
	    }
	    else {
		print "failed\t$id\t$page\t$lastPage\n";
	    }
